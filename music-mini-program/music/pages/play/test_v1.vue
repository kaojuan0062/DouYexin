<template>
	<view class="play">
		...
		<!-- 底部弹窗播放列表 -->
		<uni-popup ref="popup" type="bottom">p 内容</text>
			</view> -->
...
<script>
	...
	import audioUtils from '../../common/audioContext.js'
	export default {
		...
		onLoad(options) {
			//播放器设置
			this.isPlaying=audioUtils.getIsPlaying();
			this.isPlayEnd=audioUtils.getIsPlayEnd();
			this.currentTime=audioUtils.getCurrentTime();
			this.currentTimeStr=audioUtils.getCurrentTimeStr();
			this.duration=audioUtils.getDuration();
			this.timeStr=audioUtils.getTimeStr();
		...
		methods: {
			...
			createAudio() {
				...
				var innerAudioContext = this._audioContext = audioUtils.getAudioContext(this.thisSong);
				//var innerAudioContext = this._audioContext = uni.createAudioContext();
				//this._audioContext=innerAudioContext;
				//this.isPlaying = true;
				this.duration=audioUtils.getDuration();
				this.timeStr=this.formatTime(this.duration)
			...
			nextSong: function(id) {
				...
				var preSrc = this._audioContext._src;
				this.pause();
				...
				getThisSong(this.thisId).then(res => {
					if (res != null && res != undefined) {
						...
						console.log("下一首"+this._audioContext.src);
						// // this._audioContext._src= this.thisSong.url;
